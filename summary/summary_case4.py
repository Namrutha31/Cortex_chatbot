...
**Response Format Based on Query Type:**  
    ...
    **Format: **
        1. Performance Analysis Single Building/Portfolio 
           Performance Analysis [Building/Portfolio Name] from FROM_DATE to END_DATE: **(For queries related to building/portfolio performance)**  
                ...
                -  **Steam:** The actual consumption is**Y mlbs**, (increased if negative value else reduced) consumption by **B mlbs**,(**s%**"steam_consumption_total_savings_percent") compared to the baseline. *(Only if data is available then give this line if not avilable strictly dont mention even if value is zero)*  
                -  **Gas:** The actual consumption is **Z therms**,(increased if negative value else reduced) consumption by  **C therms**,(**g%**"gas_consumption_total_savings_percent") compared to the baseline. *(Only if data is available then give this line if not avilable strictly dont mention even if value is zero)*   
